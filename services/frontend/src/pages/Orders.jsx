...
export default function Orders() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        {orders.length === 0 ? (
          <p className="text-slate-500 text-sm">No orders yet. Place your first trade in Markets!</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-slate-500 border-b border-slate-800">
                  ...
                  <th className="pb-3 text-right font-medium hidden md:table-cell">Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {orders.map(o => (
                  <tr key={o.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-2.5">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${o.type === 'BUY' ? 'bg-green-900/40 text-green-400' : 'bg-red-900/40 text-red-400'}`}>
                        {o.type}
                      </span>
                    </td>
                    ...
                    <td className="py-2.5 text-right">
                      <span className="text-xs bg-green-900/30 text-green-400 px-2 py-0.5 rounded-full">{o.status}</span>
                    </td>
                    <td className="py-2.5 text-right text-slate-500 hidden md:table-cell text-xs">
